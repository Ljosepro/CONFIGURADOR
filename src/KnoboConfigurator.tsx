...
const MidiConfigurator: React.FC<{ onProductChange?: (product: 'beato' | 'knobo' | 'mixo' | 'beato16') => void }> = ({ onProductChange }) => {
...
    // Estados de React
  const [currentView, setCurrentView] = useState<'normal' | 'chasis' | 'buttons' | 'knobs'>(() => {
    const saved = localStorage.getItem('knobo_currentView');
    return saved ? (saved as 'normal' | 'chasis' | 'buttons' | 'knobs') : 'normal';
  });
...
            else if (meshName.includes('knob') && !meshName.includes('knobo-02')) {
        // Para knob1, knob2, knob3, knob4 específicos en la colección knobs, aplicar material rosa
        if (meshName === 'knob1' || meshName === 'knob2' || meshName === 'knob3' || meshName === 'knob4') {
          ...
          console.log('Knob procesado:', child.name, 'con material rosa');
        } else {
          // Para otros knobs, mantener la lógica original
...
  return (
  <PayPalScriptProvider options={{ clientId: PAYPAL_CLIENT_ID, currency: "USD" }}>
    <div className="w-full h-screen bg-black text-gray-200 overflow-hidden relative">
      ...
      {/* Botón de reset (centro-derecha) */}
      <div style={{ position: 'fixed', top: 16, right: 24, zIndex: 51 }}>
        <button
          className="px-5 py-2 bg-red-500 text-white border border-red-500 rounded font-bold text-sm uppercase tracking-wide transition-all duration-200 hover:bg-red-600 hover:border-red-600 hover:-translate-y-0.5 hover:shadow-lg shadow-[0_0_8px_2px_#ef444480,0_0_16px_4px_#0ff5]"
