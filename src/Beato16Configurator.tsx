...
const Beato16Configurator: React.FC<{ onProductChange?: (product: 'beato' | 'knobo' | 'mixo' | 'beato16') => void }> = ({ onProductChange }) => {
  ...
  const handleAddToCart = useCallback(() => {
    // --- 1. ID de producto de Wix (debes cambiarlo por el ID del Beato16) ---
    const productId = "ID_DEL_PRODUCTO_BEATO16_EN_WIX"; 

    // --- 2. Lógica de opciones (sin precio) ---
    // El precio lo define Wix basado en esta opción.
    const paqueteElegido = "Paquete Pro"; 
    ...
    // --- 4. CREAR EL OBJETO PARA ENVIAR A WIX (sin precio) ---
    const cartData = {
      productId: productId,
      quantity: 1,
      options: {
        ...
        customTextFields: [{
          ...
          value: `Chasis: ${chasis}, Botones: ${botones}, Knobs: ${knobs}, Teclas: ${teclas}, Faders: ${faders}`
        }]
      }
    };

    // --- 5. CORRECCIÓN DE SEGURIDAD: Enviar el mensaje a un origen específico ---
    // IMPORTANTE: Reemplaza "https://www.tu-sitio-wix.com" con la URL real de tu página en Wix.
    const wixTargetOrigin = "https://www.tu-sitio-wix.com";

    if (window.parent) {
      window.parent.postMessage(cartData, wixTargetOrigin);
      console.log("Datos del producto enviados a Wix (de forma segura):", cartData);
    }
  ...
  const sendConfigToWix = useCallback(() => {
    ...
    if (window.parent) {
      // CORRECCIÓN: Usa un origen específico.
      const wixTargetOrigin = "https://www.tu-sitio-wix.com";
      window.parent.postMessage(chosenColors, wixTargetOrigin);
    }
  }, [chosenColors]);
  ...
  const prepareModelParts = useCallback((model: THREE.Group) => {
    ...
    const newChosenColors: ChosenColors = {
      type: 'configUpdate', chasis: 'Azul', buttons: {}, knobs: {}, teclas: {}, faders: {}
    };
    ...
    model.traverse((child: THREE.Object3D) => {
      ...
      if (meshName.includes('cubechasis')) {
        child.material = new THREE.MeshStandardMaterial({ color: PALETTES.chasis['Azul'].hex, metalness: 1, roughness: 0.6 });
    ...
    try {
      ...
      loader.load('./models/beato16.glb', (gltf: any) => {
        ...
        model.traverse((child: any) => {
          if (child.isMesh && ((typeof child.name === 'string' && child.name.toLowerCase().includes('boton')) || (typeof child.name === 'string' && child.name.toLowerCase().includes('aro')))) {
  ...
  const menuIcons = [
    ...
    { id: 'chasis', icon: `M4 4 Q2 4 2 8 L2 24 Q2 28 4 28 L28 28 Q30 28 30 24 L30 8 Q30 4 28 4 Z M8 8 A1.3 1.3 0 1 1 10.6 8 A1.3 1.3 0 1 1 8 8 M12 8 A1.3 1.3 0 1 1 14.6 8 A1.3 1.3 0 1 1 12 8 M16 8 A1.3 1.3 0 1 1 18.6 8 A1.3 1.3 0 1 1 16 8 M20 8 A1.3 1.3 0 1 1 22.6 8 A1.3 1.3 0 1 1 20 8 M5 15 A2 2 0 1 1 9 15 A2 2 0 1 1 5 15 M11 15 A2 2 0 1 1 15 15 A2 2 0 1 1 11 15 M17 15 A2 2 0 1 1 21 15 A2 2 0 1 1 17 15 M23 15 A2 2 0 1 1 27 15 A2 2 0 1 1 23 15 M5 22 A2 2 0 1 1 9 22 A2 2 0 1 1 5 22 M11 22 A2 2 0 1 1 15 22 A2 2 0 1 1 11 22 M17 22 A2 2 0 1 1 21 22 A2 2 0 1 1 17 22 M23 22 A2 2 0 1 1 27 22 A2 2 0 1 1 23 22`, title: 'Chasis', viewBox: '0 0 32 32' },
    ...
    <PayPalScriptProvider options={{ clientId: PAYPAL_CLIENT_ID, currency: "USD" }}>
      <div className="w-full h-screen bg-black text-gray-200 overflow-hidden relative">
        ...
        <div className="absolute top-6 left-1/2 transform -translate-x-1/2 z-10 flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold leading-none m-0" style={{ fontFamily: 'Gotham Black, Arial, sans-serif', color: '#fff', textShadow: '0 0 12px #a259ff, 0 0 24px #0ff, 0 0 2px #fff', letterSpacing: '0.04em' }}>CONFIGURADOR</h1>
        </div>
        ...
        <div className={`fixed top-0 right-0 h-screen border-l border-gray-700 shadow-2xl transition-all duration-400 flex overflow-hidden z-10 ${currentView === 'normal' ? 'w-28' : 'w-[320px]'}`} style={{ backgroundColor: '#2D3559', boxShadow: '0 0 32px 4px #a259ff, -12px 0 32px 0 #a259ff', zIndex: 10 }}>
          <div className="w-28 p-6 flex-shrink-0" style={{ paddingTop: '200px' }}>
            <div className="flex flex-col gap-2">
              {menuIcons.map(({ id, icon, title, viewBox }) => (
                <button key={id} onClick={() => changeView(id as any)} className={`w-full aspect-square border rounded flex items-center justify-center p-3 transition-all duration-200 text-white ${currentView === id ? 'bg-[#6C4FCE] border-[#6C4FCE] shadow-[0_0_8px_2px_#6C4FCE80,0_0_16px_4px_#0ff5,0_0_2px_1px_#fff3]' : 'border-gray-600 bg-[#A693E5] bg-opacity-100 hover:bg-[#6C4FCE] hover:border-[#6C4FCE]'}`} title={title}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox={id === 'chasis' ? '0 0 32 32' : id === 'knobs' ? '0 0 32 32' : id === 'faders' ? '0 0 24 24' : '0 0 24 24'} className="w-4/5 h-4/5 mx-auto my-auto fill-white text-white" fill="#fff"><path d={icon} /></svg>
                </button>
              ))}
              ...
              <button type="button" onClick={() => document.getElementById('sidebar-upload')?.click()} className="w-full aspect-square border rounded flex items-center justify-center p-3 transition-all duration-200 text-white bg-[#6C4FCE] border-[#6C4FCE] shadow-[0_0_8px_2px_#6C4FCE80,0_0_16px_4px_#0ff5,0_0_2px_1px_#fff3] hover:bg-[#8e6cfb] hover:border-[#8e6cfb]" title="Subir archivos de personalización">
            ...
            <div className="mt-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-4 gap-2 p-2 rounded overflow-x-auto animate-scaleIn" style={{ backgroundColor: '#232846' }}>
                {Object.entries(getCurrentColors()).map(([name, colorData], index) => (
                  <div key={name} className="w-10 h-10 rounded-full cursor-pointer border-2 border-[#a259ff] shadow-[0_0_6px_1px_#a259ff55] transition-all duration-200 shadow-inner hover:scale-110 animate-fadeInUp" style={{ backgroundColor: colorData.hex, animationDelay: `${index * 50}ms` }} title={name} onClick={() => applyColor(name, colorData)} />
                ))}
              </div>
            </div>
          </div>
