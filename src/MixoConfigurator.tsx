...
const MixoConfigurator: React.FC<{ onProductChange?: (product: 'beato' | 'knobo' | 'mixo' | 'beato16') => void }> = ({ onProductChange }) => {
  ...
  const prepareModelParts = useCallback((model: THREE.Group) => {
    ...
    const newChosenColors: ChosenColors = { type: 'configUpdate', chasis: 'Azul', buttons: {}, knobs: {}, faders: {} };
    ...
    <PayPalScriptProvider options={{ clientId: PAYPAL_CLIENT_ID, currency: "USD" }}>
      <div className="w-full h-screen bg-black text-gray-200 overflow-hidden relative">
        ...
        <div style={{ position: 'fixed', top: 16, left: 6, zIndex: 51, display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button className="px-5 py-2 bg-purple-400 text-black border border-purple-400 rounded font-bold text-sm uppercase tracking-wide transition-all duration-200 hover:bg-yellow-200 hover:border-yellow-200 hover:-translate-y-0.5 hover:shadow-lg" onClick={() => window.location.href = 'https://www.crearttech.com/'}>Inicio</button>
          <div style={{ background: 'linear-gradient(180deg, #6C4FCE 0%, #291A57 100%)', borderRadius: '12px', padding: '8px 24px' }}><h1 className="text-2xl text-white font-bold leading-none m-0 text-shadow" style={{ fontFamily: 'Gotham Black, Arial, sans-serif' }}>MIXO</h1></div>
        </div>
        ...
        <div className={`fixed top-0 right-0 h-screen border-l border-gray-700 shadow-2xl transition-all duration-400 flex overflow-hidden z-10 ${currentView === 'normal' ? 'w-28' : 'w-[320px]'}`} style={{ backgroundColor: '#2D3559', boxShadow: '0 0 32px 4px #a259ff, -12px 0 32px 0 #a259ff', zIndex: 10 }}>
          <div className="w-28 p-6 flex-shrink-0" style={{ paddingTop: '200px' }}>
            <div className="flex flex-col gap-2">
              {menuIcons.map(({ id, icon, title, viewBox }) => (<button key={id} onClick={() => changeView(id as 'normal' | 'chasis' | 'buttons' | 'knobs' | 'faders')} className={`w-full aspect-square border rounded flex items-center justify-center p-3 transition-all duration-200 text-white ${currentView === id ? 'bg-[#6C4FCE] border-[#6C4FCE] shadow-[0_0_8px_2px_#6C4FCE80,0_0_16px_4px_#0ff5,0_0_2px_1px_#fff3]' : 'border-gray-600 bg-[#A693E5] bg-opacity-100 hover:bg-[#6C4FCE] hover:border-[#6C4FCE] hover:shadow-[0_0_4px_1px_#6C4FCE60,0_0_8px_2px_#0ff3]'}`} title={title}><svg xmlns="http://www.w3.org/2000/svg" viewBox={viewBox || '0 0 24 24'} className="w-4/5 h-4/5 mx-auto my-auto fill-white text-white" fill="#fff"><path d={icon} /></svg></button>))}
